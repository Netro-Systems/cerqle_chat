...
class _MessageContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isImage = message.type == CerqleMessageType.image ||
        (hasMedia &&
            _isImageAttachment(
    ...
    final isAudio = message.type == CerqleMessageType.audio ||
        (hasMedia &&
            _isAudioAttachment(
              filename: attachment?.filename ?? localUpload?.filename,
              mimeType: attachment?.mimeType ?? localUpload?.mimeType,
...
class _ImageAttachmentPreview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isPending = message.status == CerqleMessageStatus.pending;
    final isFailed = message.status == CerqleMessageStatus.failed ||
...
class _FileAttachmentBubbleState extends State<_FileAttachmentBubble> {
  ...
  Widget build(BuildContext context) {
    ...
    final failed = widget.message.status == CerqleMessageStatus.failed ||
        widget.message.status == CerqleMessageStatus.unconfirmed;
...
Future<void> _openDocumentAttachment(
  ...
  try {
    if (upload != null) {
      final cerqleTemp = Directory('${Directory.systemTemp.path}/cerqle');
      if (!await cerqleTemp.exists()) {
        await cerqleTemp.create(recursive: true);
      }
...
Future<void> _downloadAttachmentToDevice(
  ...
  try {
    ...
    // Strategy 1: Save directly into standard "cerqle" folder inside Downloads
    ...
    for (final baseDir in candidateParentDirs) {
      try {
        if (await baseDir.exists()) {
          final cerqleDir = Directory('${baseDir.path}/cerqle');
          if (!await cerqleDir.exists()) {
            await cerqleDir.create(recursive: true);
          }
          final targetFile = File('${cerqleDir.path}/$filename');
    ...
    if (savedPath != null) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('File saved to cerqle: $filename'),
